...
output "argocd_server_url" {
  description = "LoadBalancer hostname for the argocd-server svc (may take a few minutes to populate after apply)"
  value       = try(data.kubernetes_service.argocd_server.status[0].load_balancer[0].ingress[0].hostname, "not ready yet - re-run terraform apply or check kubectl get svc -n argocd")
}
